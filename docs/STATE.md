...
> - **The voice gets its own strength, defaulting to 1.4.** The face file's audio-branch deltas are noise and at equal strength they are LOUDER than the voice file's signal (median ‖D‖ 1.45 vs 1.10). `character_voice_strength` is asserted into `params` in the same commit that adds it — the allowlist trap, closed with a test rather than a hope. **Owner's ear still gates the default**: the ladder is rendered and on `clipgrade`; the promote checklist gains "voice ladder verdict received".
