...
> - **Inert in the public tree.** `ANALYTICS_KEY_DEFAULT = ""`, and an empty key means **no socket is ever opened** — not "a request that fails". A fresh clone sends nothing, forever, until the owner pastes a PostHog project key. This is the property that makes shipping it defensible after the 2026-05 opt-in module was reverted (`da1d6f5`, "not going to be well accepted in the open source world").
