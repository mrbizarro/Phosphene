> **🎞 2026-09-24 — v4.16.1: 720p/1080p exports fill the frame instead of padding black bars.** Same X user as 4.15.3 (M5 Pro 48 GB, on 4.15.3), after posting H3 numbers (Draft 3 s 37 GB 2m32s, Standard 36 GB 3m53s, High 39 GB 13m27s — recorded in docs/H3_ENGINE.md as field data): "Is it a bug that black bars appear on the left and right when the resolution is set to 720p?" (H3 Image→Video from a photo). **Cause:** not the reference fit — `_h3_fit_first_frame` already cover-crops the reference onto the tier canvas — but the export pass: `compute_upscale_plan` fitted any non-16:9 source INSIDE 1280×720 and padded. H3 Draft is 640×384 (5:3) → **40 px black each side**, Standard 768×448 (12:7) → 23 px; luma-scanned on two real exports in the owner's outputs (L=40/R=40 and L=22/R=26) and reproduced with a portrait and a 16:9 reference through the real `_h3_fit_first_frame` + both filtergraphs (4.16.0 bars, 4.16.1 none). LTX had the same shape on 1280×704 (8 px top/bottom); LTX i2v otherwise snaps its canvas to the image's orientation and Balanced is exact 16:9. The 2026-08-06 entry called the pillarbox "correct — no crop, no distortion"; for a file whose job is to be 720p it wasn't, and the app already crops rather than pads elsewhere (reference fit, Extend's downscale ≤31 px). **Fix:** `EXPORT_FILL_MAX_TRIM = 0.08` — a source within 8% of the target aspect is scaled to cover + centre-cropped (`scale=…:force_original_aspect_ratio=increase,crop=W:H`); plan gains `fill`, `trim_w`, `trim_h`. 5:3→720p trims 24 px top/bottom, 12:7 13 px, 1280×704 14 px left/right, Native 7:4→1080p 8 px. Square/4:3 keep bars. The Sharp (PiperSR) script carries the same constant (a test holds them equal) and now gets the source dims. Export notes, tier blurbs, the pill sub-labels ("scale to fill"), H3/LTX log lines and webapp/docs/video.md updated. No face detection ships with the base install (cv2 is not in a fresh venv), so the crop is centred; the trims are ≤ 3.4% per edge. One Codex pass (codex-cli 0.155.1, read-only): P2 — LTX `i2v_clean_audio` pads 1280×704→1280×720 at the mux BEFORE the export, so the fill cropped a frame that already had bars and the plan lied (fixed: the mux skips its pad when the export will fill; with export off the pad stays); P3 — "no crop" still promised in two Export hints and the clip-details rows (fixed: the rows read the sidecar plan — "filled, edges trimmed" / "bars added" / "no crop"). New `test_export_fill_no_bars.py` (12; the 10 original ones all fail on 4.16.0; one encodes a real 640×384 clip through both targets and checks the edge columns/rows are not black). Not changed: storyboard film assembly still pads mixed-aspect clips on purpose (`test_storyboard_assembly` pins "bars, never crop").

> **🎼 2026-09-24 — v4.16.0: the Music Studio line goes public, with the 4.16.0 review fixes.** Codex reviewed seven flows of beta `cdca462` (install, jobs, LTX, H3, storyboard/editor, music, UI): 69 findings, fixed on three branches (`fix416-jobs` 17, `fix416-editor` 25 incl. one extra editor bug, `fix416-install` 27 + the FP16 flag; JOB-05 = INST-07), each with a test that fails before its fix. Integrated as `int-4160` on beta `33dd394` (after the 4.15.4 merge-back). **Conflicts:** only the UI-01 pair (`panel/routes_files.py`, `test_image_route_scope.py`), resolved to the 4.15.4 version (wider raster allowlist, never svg) keeping fix416-install's trash (UI-05) change; mlx_ltx_panel.py, queue.js, characters.js and image_engine.py auto-merged. **One Codex pass on the combined diff** (gpt-6-astra, high, read-only): 2×P2, both fixed — Load Params on a pre-4.15.2 a2v clip restored the legacy float 1.0 as an explicit "1" (audio guidance off on Q8; now reopens as Auto, `test_review_416_ltx.py`), and an incomplete FLAT H3 weights tree (LTX_H3_MODELS) was skipped by Install/Repair forever (now `h3_fetch_weights` in `h3_roots.sh` stops with an `error:` naming the missing parts; install_h3.js dispatch 16 chars). **Fleet fix:** "The reference image can't be read (ModuleNotFoundError)" was a half-installed engine venv (panel cannot import PIL), not a bad picture — 6 H3 i2v failures, one 4.15.2 install; `h3_reference_image_error()` now gives ENGINE_ENV_REPAIR (venv_broken). **H3 FP16 decode ON by default** after the release A/B through the panel (High 5 s Turbo, seed 42, runner 7919020): decode 90.1 → 77.4 s, peak 10.7 → 9.5 GiB, no visible difference (RGB PSNR 42 dB through the h264 exports; mean |Δ| 1.4/255). **LTX-03 verified on the GPU:** FFLF (Q8 dev, 768×384/49f) and Q4 A2V (Compact tier, 512×256/49f) with Pixar_Toon attach 576/576 modules and change the clip (PSNR vs no-LoRA 30.3 / 25.4 dB); a repeat no-LoRA A2V is bit-identical (inf), so the LoRA cache key releases the adapter. **Tests:** full root pytest in one process = the same suite-order pollution as beta/main (route tests pass alone, fail after an earlier test rebinds panel globals; beta/main shows it on test_routes/test_uploads/test_room_tone, int adds test_trash_collisions/test_version_pull_route — all pass per file); `release_gates.sh --fast` int 143/4/2 vs beta/main 119/4/2 with the identical 4 environmental FAILs (assert_registry 152/17 and assert_schedules 40/5 need packs + the bizarrotrn character, test_yue2_train_voice needs yue2 weights, scripts/test_pack_release no-mirror case). PostHog: the 403 a fixer saw was an endpoint the query key has no scope for; HogQL queries with it return 200. **Skipped-by-fast gates run separately:** test_storyboard_editor_ui 466 passed; check_output_codec PASS on the FFLF render. **From zero:** fresh clone of the release commit + install.js code steps in a Pinokio-shaped env (miniforge PATH, MPS fallback + fast-math set, no SSL/SSH/PYTHONPATH), booted the start.js way → /version 4.16.0 not dirty, UI 200, no traceback, /image refuses the settings file, a render with no weights fails naming the missing pack. **Update path:** a 4.15.4 install cloned from a local stand-in for public main, then 4.16.0 published there and update.js run → fast-forward, 4 steps rc 0, no `error:` line, VERSION 4.16.0, /status integrity OK. **INST-03:** env replaced by an empty `uv venv --seed` (boot WARNs "has a Python but no packages"); Update again → post_update rebuilt it, `import ltx_core_mlx, ltx_pipelines_mlx, mlx, mlx_arsenal` OK, boot clean; Repair (install.js code steps) rebuilds it too. **Found on the way, NOT fixed (mirror, not code):** the weights-ltx25-v1 release asset `LICENSE-LTX-2.x-Community-License.md` was overwritten on 2026-09-06 by the ic_upscale_x2 publish (34545 B, sha a6622b0e…); q4_25/q8_25/hq_25/gemma4_25 manifests expect 30938 B (4e7dd26d…), so every mirror fetch of those packs ends "1 file FAILED" + a WARN, though every weight file verifies and the pack reads ready.

> **🔒 2026-09-24 — v4.15.4: /image serves images only (security hotfix, Codex UI-01).** `panel/routes_files.py get_image` resolved the path and checked it sat under OUTPUT / UPLOADS / STATE_DIR — and served ANY file there. STATE_DIR holds `panel_settings.json` (HF token, CivitAI key, PostHog keys, which `/settings` never returns), so a local process, another local user, or anyone reaching a LAN-exposed/proxied panel could read them with one GET. A cross-origin web page could trigger the request but not read the response (no CORS headers). Fix: `IMAGE_ROUTE_EXTS` (png/jpg/jpeg/jpe/jfif/webp/gif + bmp/tif/tiff/avif/heic/heif, because the pickers accept `image/*`; **never svg** — same-origin script) checked right after the root check, before the file is opened, so the thumbnail path (w=) cannot fall back to serving the original either. Fixed first on beta branch fix416-install (f0a380a, image set widened here), ported alone onto public 4.15.3. Test `test_image_route_scope.py` (settings/queue/fleet/no-extension files 403 with and without w=, sidecar JSON in outputs/uploads 403, png/JPG/bmp/tiff still 200 from every root, svg 403, /etc/hosts 403); fails 3/6 on 4.15.3. Other path routes re-checked: /file is OUTPUT-only, /sidecar is media-suffix-gated OUTPUT/UPLOADS, /loras/download is .safetensors under the loras dir. One Codex pass (gpt-6-astra, read-only): leak closed (symlink/case/trailing-dot/double-ext/thumbnail fallback all checked); P3 .jfif/.jpe uploads would have 403d -> added; 2xP2 need a planted symlink in the thumb cache or next to an output (attacker already has file write as the user) -> not a boundary, left.

> **🩹 2026-09-24 — v4.15.3: Hailuo H3 on a 36-59 GB Mac, said plainly; public pinokio.js identity.** A Japanese user with a 48 GB Mac wrote on X: "Since it requires 64 GB, I couldn't even install it [H3] from the UI." Every GATE on 4.15.2 would have let that Mac install and render H3 (pinokio.js `H3_MIN_BYTES` 36e9, `h3_preflight.sh` 36e9, panel `H3_MIN_RAM_GB_Q8` 36, Q8 lane peak 25.63 GiB). Reproduced against 59d733d with a fake `sysctl` answering hw.memsize=51539607552 (48.0 GiB) on PATH, panel booted the Pinokio way: `/status.h3` = capable true, available false, needs_q8_dit true, reason not_installed — and **`size_note` = "~75 GB · needs 64 GB unified memory"**, a literal unchanged since v3.4.0 (0ef1a98) that every earlier 46→36 sweep missed. That line sits under the pitch on the H3 install card (engine switcher + the Settings card's "How to enable H3" + the Models row). Second half of the dead end: the card says "click Install Hailuo H3 in the Pinokio sidebar", and while the panel runs pinokio.js returns the Open Panel menu, which had **no H3 entry** — it existed only with the panel stopped. Third: the Settings card and `ram_note` told a Mac that never installed H3 "its low-RAM engine isn't built yet … no extra download". Fourth: a 36-59 GB Mac holding the weights but not the Q8 build was told to click "Install Hailuo H3", which pinokio.js hides once `h3_ready`. **Fixes:** `h3_size_note()` from the constants ("needs 36 GB+ … 60 GB+ also unlocks the full bf16 engine"); `h3_ram_verdict()` splits the band by `h3_paths()` the same way pinokio.js picks its one H3 entry (nothing complete → Install, weights_ok but venv/runner missing → Repair, all present but no Q8 pack → Build; all keep "on its reduced-RAM lane" so `_analytics_refusal_reason` still says h3_ram); `H3_INSTALL_MENU_TEXT` / `H3_REPAIR_MENU_TEXT` / `H3_BUILD_MENU_TEXT` quoted verbatim by pinokio.js; pinokio.js `h3Offer()` puts Install/Repair/"Build Hailuo H3 compact engine (weights kept — no re-download)" in the running AND stopped menus (build entry = h3_ready && no h3-dit-q8 pack && totalmem < 60e9); install card adds "This Mac runs it." on the q8 lane and says the entry is there while the panel runs; Settings card is an offer (download icon, no warn) unless reason is missing_q8_dit; repair card uses ram_note when missing_q8_dit (its missing-list is empty). `h3_build_q8.sh` now waits (30 s polls, 60 min cap, fail-open) while `state/panel_queue.json` has a `current` job and a panel process exists, before the quantizer and before the fresh-process `load_dit` validation — both load the whole ~20 GB Q8 DiT, and the install is now reachable mid-session (residual: a render started DURING the ~1-2 min validation is not blocked). One Codex pass (codex-cli 0.155.1, read-only): 2×P2 — the build sentence could name a Build entry the menu doesn't show when the venv/runner is broken (fixed: menu-aligned sentences + test), and install-while-rendering could stack ~20 GB on a render (fixed: the wait above). Docs: README memory row, H3_ENGINE.md bands 46→36, troubleshooting #h3-compact, preflight "LTX-2.3" → "LTX". **BETA identity:** public pinokio.js carried `title: "Phosphene BETA"` + "[BETA — unreleased builds … port 8199]" since **v4.8.2 (4e1dde6, 2026-09-01)** — the beta checkout's local edit got committed to dev and rode every promote. On Pinokio 8.2.0 users did NOT see it in the sidebar: `kernel/api meta()` and the server's folder view overlay pinokio.json's title/description (Object.assign(pinokio.js, pinokio_meta.json, pinokio.json)), and public pinokio.json says "Phosphene"; it was latent text (app search index, raw file, any Pinokio without the overlay). Restored "Phosphene" / "[MAC ONLY] …". The beta install's sidebar name comes from its own skip-worktree **pinokio.json** ("Phosphene BETA"), which nothing upstream touches. **Beta checkout note:** phosphene-dev.git has pinokio.js skip-worktree; ANY upstream pinokio.js change aborts a pull there ("Please commit your changes or stash them") — already true for the pending 3-commit ff (4.15.2 changed pinokio.js). Its working pinokio.js holds nothing local any more, so at the next panel restart: `git update-index --no-skip-worktree pinokio.js && git checkout -- pinokio.js && git merge --ff-only beta/main` (leave pinokio.json and start.js skip-worktree). New test `test_h3_48gb_install_path.py` (16: panel sentences at 48 GiB both ways, verbatim menu strings, node-evaluated pinokio.js menus on a fake installed root at 32/48/64 GB running and stopped, public identity, preflight at 48 GB via fake sysctl). Fleet read (PostHog, since 09-22): 94/132 active installs on 4.15.2, failure rate 7.8%; 48 GB is not blocked in data — H3 on 24/60 active 48 GB installs, 203 H3 renders / 1 failure; the reporter is a pre-install drop-off analytics cannot see. Open from fleet: new-install "engine venv has a Python but no packages" rose 1.8% → 7.5% (5/67; two M5 Max 128 GB never recovered) — cause not reproduced; oversized a2v on 8 GB not refused (Metal watchdog). Issues: 7 open, none an in-reach bug (#62 retrain still owed; #61/#78/#48 await reporters; #24/#63/#83 features). UI is English-only; the reporter is Japanese — noted, no i18n built.

# Phosphene — project state, history, open work

> **🎤 2026-09-22 — v4.15.2: lip-sync on LTX audio-to-video follows the audio again.** The owner saw the improved lip-sync on the study's shot and ordered it shipped, because LTX lip-sync was not good. The order covered the lip-sync fix and nothing else from dev.
>
> **The bug was a number with two meanings.** The Audio tab's *Audio conditioning strength* feeds two pipelines. Q4 distilled multiplies the audio tokens by it, so 1.0 is the identity and audio is fully on. The Q8 two-stage pipeline has no such parameter; since **v4.8.1** the helper reroutes the slider onto the stage-1 guider's `modality_scale`, whose term is `(value − 1) × (cond − uncond_modality)` — at **1.0 exactly zero**. The engine's own value there is 3.0, which is also what every Q8 a2v render used before 4.8.1 (the slider was being dropped by the kwarg filter then). `make_job` stamped 1.0 on every job and the slider started at 1.0, so every Q8 a2v clip rendered at the default since 4.8.1 had audio guidance switched off.
>
> **What shipped.** `make_job` no longer fabricates a number; `a2v_audio_scale()` resolves a blank to the lane's own value (Q8 3.0, Q4 1.0) in the a2v branch, against `uses_q8`, so the Q8→Q4 fallback gets the Q4 value. Explicit values pass through on both lanes; `nan`/`inf`/`<= 0` fall back. The slider starts on **Auto** and sends nothing until dragged. The run log says `scale=3.0 (Q8 default)`, and the sidecar records `audio_conditioning_scale_used`. The helper's Q4 branch got the same `<= 0` guard (a negative value used to flip the sign of every audio token). **Jobs saved by 4.15.1** carry the old default as a float 1.0 and survive Update in the saved queue, and `/queue/retry` copies params verbatim. `a2v_requested_scale()` reads a float 1.0 as "no request". Today's `make_job` only stores strings, so a float can only come from an old job, and a new job that sends `"1.0"` keeps 1.0. **Also shipped:** the `pinokio.js` probe that offers *Repair Phosphene engine (models kept)* when the venv has an interpreter but no `ltx_pipelines_mlx`. 4.15.1's notes and its `test_engine_env_fault` announced it, but the edit sat in the dev checkout under `skip-worktree` and never reached a commit. The test was failing in the public 4.15.1 tree.
>
> **Deliberately not shipped** (dev has them, the next minor can carry them): the vocal-stem seam and `scripts/pinokio/a2v_stems_deps.sh`, the a2v prompt rules, and face-tight casting. The stem needs demucs + torch (~2.5 GB) pip-installed into the engine venv; the installer has never been run and has no in-app surface. The prompt rules live in the storyboard planner and `music_video.py`, which public users cannot reach with a2v (`music_video.py` is not on public main, and the public planner never emits a2v). Face-tight casting belongs to the music-video planner.
>
> **Validation.** Codex, one pass, two findings, both fixed: P2 was the saved-job case above, P3 was a test that passed on the parser's name alone. The helper parsers are now ast-extracted and executed, and `run_job_inner`'s a2v branch is driven for real with a stub helper across new/legacy × Auto/explicit × Q8/Q4; a mutation that drops the legacy rule fails it. Gates `--fast`: **111 pass, 1 fail, 2 skip**. The fail is `assert_registry.py`, and all eight of its failing asserts are about **LTX-2.3 packs, which this Mac no longer has**. They fail identically on an untouched v4.15.1 worktree run the same way. `test_storyboard_editor_ui` (466 tests) and `check_output_codec` (on the validation clip) ran separately and both pass. Clean room: a fresh clone of public **v4.15.1**, install.js's engine steps under a Pinokio-8.2.0-shaped env with empty caches, then **Update run the way update.js runs it**. It pulled 4.15.2, and `post_update` fetched the LTX-2.5 Q4 pack and text encoder from zero. The Q8 pack was linked because this release does not touch weights. It booted under `PYTORCH_ENABLE_MPS_FALLBACK=1 PYTORCH_MPS_FAST_MATH=1` with no `SSH_*`/`PYTHONPATH`: `/version` 4.15.2, not dirty, Auto slider served. One real a2v render through `/queue/add` with no scale field (the study's reference shot: same image, 10.05 s window from 8.4167 s, seed 1901872887, 1024×576, 241 f) logged `scale=3.0 (Q8 default)` and `modality_scale=3.00`. It took **23.5 min against 18.7 min** for the same shot at the old default, because the audio pass that 1.0 skipped runs again. The video stream is **bit-identical** (PSNR ∞) to the study's acs-3.0 render the owner judged.
>
> **The metric, honestly.** On the study's scorer (mouth aperture vs vocal-band envelope, per-second, best lag) the old default scores **−0.065** and 4.15.2 scores **+0.128**, exactly as the study found. But the study's negative control does not hold for music. It scored the clip against the song shifted +3 s, and at ~143 BPM two bars are 3.34 s, inside the ±0.4 s lag search. Scored that way, the new clip gets **+0.366**. The proper null scores each clip against **every** 10.05 s window of the song. Against that null, neither number is distinguishable from chance on one 10 s clip. Full mix: old −0.065 vs null +0.077±0.091 (z −1.55), new +0.128 vs +0.118±0.095 (z +0.10). Demucs vocal stem: old −0.088 (z −1.43), new +0.175 (z +0.56). The direction holds, but the proof is the mechanism (a zero guidance term) and the owner's eye, not this number. A lip-sync metric that can prove itself needs more clips or a SyncNet-class model.
>
> **Dev side (not in the public entry).** Public 59d733d is merged back here, so `git log dev..origin/main` is empty; dev's own copy of the a2v changes, with the stem seam, prompt rules and face casting, is kept, and 2cc4b3a ports the legacy-job rule, the finite guard and the sidecar record onto it. In `phosphene-dev.git`, `pinokio.js` and `start.js` are marked `skip-worktree`. The `pinokio.js` edit is now committed with the same content, so its flag can be cleared (`git update-index --no-skip-worktree pinokio.js`) when that checkout is fast-forwarded. `start.js` keeps its flag: its `LTX_H3_*` paths are local-only and must never be committed. The lip-sync study's `+3 s` negative control is superseded by the permutation null above.
>
> **🎤 2026-09-22 — A TRAINED VOICE THAT SINGS THE WORDS YOU GIVE IT (`scripts/music/yue2_train_voice.py`, UNRELEASED).** The previous round's AR LoRA moved vocal identity a long way (sim 0.846 vs 0.657 control) but pronounced **none** of a set of unseen lyrics — 0/8 planted words, whisper word-error **1.000** — and the study concluded a trained voice and a supplied ABC score were mutually exclusive. Both findings are now superseded by measurement.
>
> **F1, new lyrics: 0/8 → 8/8, word error 1.000 → 0.214.** Two causes, and neither alone is enough. (1) **The training pair was a lie.** The prototype put the WHOLE song's lyrics in the prefix against a RANDOM 2048-token window of the track's codes; those describe different seconds, so the model could only learn "this style, this singer → codes" and never which words made which sounds. Recordings are now cut into 20.48 s windows (Maestro's 512 frames) and each window carries the lines whisper located inside it. (2) **The adapter was being driven at 1.0.** At full strength it overrides the base model's lyric conditioning completely — 0/8 either way it is trained. The 2×2, same seed, same unseen sheet, Direct mode: whole-song lyrics @1.0 **0/8 / WER 1.000**, whole-song @0.7 **4/8 / WER 1.000**, windowed @1.0 **0/8 / WER 0.896**, windowed **@0.7 8/8 / WER 0.214**. The stock model scores 7/8 at WER 0.325, so the best cell is **more accurate at the given words than the base model**, at voice similarity **0.8205** against stock's 0.7718 and a register within a semitone of the target. The WER column is what separates the two recipes: at 0.7 the whole-song adapter sings MORE words (243 vs 180) and still scores 1.000 — it learned to sing, not to sing what it is given. 0.5 is too far the other way (lyrics survive, voice does not: 0.7272, register −6.25 st).
>
> **F2 was a false premise, and disproving it produced the best take of the run.** `MAESTRO_VOICE_STUDY.md` §6.1 recorded that a real-audio-dialect LoRA forces Direct generation. That is a Maestro **UI** convention: `SongRequest.__post_init__` refuses `abc` only when `cot == "off"`, and `--mode melody` is not that — our runner has no such guard either. Handed the unplugged score directly, the direct-dialect voice sings it: **in-tune (octave-folded) 83.1 %, and in the WRITTEN octave 76.6 %**, against the stock voice's 96.1 % folded but **1.3 %** absolute — stock drops the whole melody an octave (175.5 Hz, −9.15 st), which is the problem the r3 round spent its effort transposing around. All 77 notes judged on both. The trained voice holds **6/8 lyrics and 0.8161 voice similarity at the same time**, which Direct mode never manages at full strength: a supplied score anchors the melody, leaving the lyric conditioning intact. **The shipped advice is therefore: use a trained voice WITH a score.**
>
> **The training curve says it generalises rather than memorises.** 35 windows, 400 steps, 21.8 min: artist CE 5.534 → 0.753, where the prototype's single memorised sequence was at **0.077** by the same step. Held-out minted CE moved 3.780 → 3.803 across the whole run, so the adapter never leaves YuE2's dialect — given a minted prefix it still predicts minted codes as well as it ever did. What breaks at full strength is generation under the trigger, not the dialect.
>
> **Shipped**: `scripts/music/yue2_train_voice.py` — `--recordings --lyrics --trigger --out [--steps --rank --lr --dialect direct|score --reg-pack]`, sequences built by the engine's own `token_prefixes` so training sees the byte-identical prefix inference builds, cross-entropy on the code tail only, Maestro's minted pack mixed in at `--artist-fraction` (`--reg-cache` converts it once so a machine without torch can train), output in the layout `yue2_lora.read_adapter` already reads plus a sidecar carrying the trigger, dialect, matching generation flags, base commit, data fingerprint and loss curve. The trainer is pure — no demucs, whisper or SheetSage2 in process; `--emit-windows` writes the window audio so those tools can be aimed at exactly the seconds each window trains on. Commits `23aabb9` `96ec533`. Gate: `test_yue2_train_voice.py test_yue2_lora.py test_yue2_tokenizer.py` — **118 passed**.
>
> **Two process scars, both now fixed in the scripts.** The overnight run produced nothing: it waited 59 minutes for the GPU, won the lock at 21:50 and died one line later because `gpu2.sh` had been rewritten at 20:51 **while a copy of it was executing** — bash reads a script incrementally by byte offset, so the running process resumed inside the new file. Never edit a file a running process is executing; write a new one. And the lock take is now `set -o noclobber` (atomic O_EXCL) rather than `[ ! -e ] && echo >`, which had already lost a race to another agent at 20:49 and left two GPU jobs briefly overlapping; the release now refuses to delete a lock it does not own.
>
> **The `score` dialect, measured — and why it loses.** Trained on 13 windows whose prefixes carried SheetSage2's `melody-vocal` transcription of the same seconds, then given the same test score: step 100 **97.4 % folded / 3.9 % absolute**, step 400 **76.6 % / 1.3 %**, registers −9.35 and −11.15 st — it follows the pitch classes and drops the octave, exactly as stock does. The cause is measured, not guessed: SheetSage2 writes the melody **+12.7 st above what was sung** (median over the 13 windows; 9 of them at +12.1 to +12.9), so every training pair taught "a written note means sing it an octave lower", while the test score is written at pitch (median MIDI 65 against the singer's 62.2). The `direct` voice under a score stays the recipe to use; the fix — transpose each training score to the sung octave, or calibrate the offset per window — is not in the trainer yet. The `score` curve also memorises far faster (artist CE **0.021** at step 400 vs the `direct` run's 0.753), because a unique score per prefix is a near-perfect key to that window's codes. Untested hypothesis worth one experiment: if YuE2's own pretraining scores share this octave convention, it would explain why the STOCK model drops a written score an octave too — the thing r3 compensated for with a −12 st transposition.
>
> **Not done**: no panel surface (CLI + the LoRA picker's `user/` folder); one recording only, so no held-out generalisation check (Maestro wants 2–50 songs and one held out); timbre still untouched — that is head + NAR adaptation and still needs an MLX VAE encoder, a differentiable NAR path and the unpublished minted anchor corpus.
> **🎤 2026-09-22 — THE A2V LANE WAS NOT LIP-SYNCING, AND THE BIGGEST REASON WAS A DEFAULT (branch `a2v-sync-fixes`, UNRELEASED).** The owner's verdict on the first real singing shot was that the mouth was not on the words. It is now a measurement, not an opinion: on one shot with the image, the 10.05 s audio window, the prompt and the seed held fixed, the per-second correlation between the inner-lip aperture (mediapipe FaceMesh 13/14, normalised by inter-ocular distance) and the 300–3400 Hz vocal-band envelope was **−0.065** — *worse* than the same clip scored against deliberately wrong audio (−0.054). Four fixes, all measured or structural, none of them the sampler.
>
> **The default meant OFF on one of the two lanes.** One slider, two pipelines, opposite meanings at the same number: Q4 distilled multiplies the audio tokens by `audio_conditioning_scale`, so 1.0 is the identity and the audio is fully on; the Q8 two-stage pipeline has no such parameter at all, so the helper reroutes the slider onto the stage-1 guider's `modality_scale`, whose term is `(value − 1) × (cond − uncond)` — **1.0 makes it exactly zero**, and the vendored engine's own default is 3.0. The panel stamped 1.0 on every a2v job, so every Q8 a2v clip ever rendered at the default had audio guidance switched off in all but name. At 3.0, nothing else changed: **−0.065 → +0.128**, positive seconds 44% → 67%. `make_job` no longer fabricates a number; a blank means "the lane's engine default" and `a2v_audio_scale()` resolves it in the one place that knows which pipeline is about to run, including the Q8→Q4 fallback. An explicit value passes through untouched on both lanes. The slider starts on **Auto** and sends nothing until it is dragged.
>
> **The model was listening to the band.** Drums, bass and guitar reach the audio encoder as energy that can be read as syllables, so it hedges — the mouth moves a little, all the time, on everything. A demucs vocal stem as the conditioning waveform was the only candidate with a positive zero-lag correlation (**+0.156**) and **doubled** the mouth movement (`mouth_std` 0.131 against 0.065). `audio_stem` / `audio_stem_auto` split what the model LISTENS to from what the clip PLAYS, and the original song is muxed back over the finished clip with the video stream copied. demucs is an optional extra (`scripts/pinokio/a2v_stems_deps.sh`, engine venv, written but not run); with it missing the auto option degrades to the full mix with a note naming the installer and never fails a queued job. Separation is cached by path+size+mtime, so a twelve-shot film separates once. `POST /music/video/plan` takes `vocal_stem`.
>
> **Our own planners were writing the lip-sync killer, mechanically, on every shot.** LTX reads a stillness word as "nothing in the scene moves" — including the singer's lips. `storyboard_planner._CAMERA_SENTENCES["static"]` is *"the frame never moves - no pan, no push-in, no reframing"* and the settle law adds *"with no new movement of any kind"*; `storyboard.py` then **skipped** prompt validation for a2v, so the audio-driven mode got less checking than every other one. The law moved into `storyboard.py` so both planners and the validator read one copy: stillness on an a2v shot is a blocking error with the phrase named, a clause about the CAMERA is deleted while a clause about a PERSON is rewritten in place ("she stands still at the microphone, singing" → "she stands steady at the microphone, singing"), the direction is capped at 40 words, and the literal sync contract is appended LAST so no polish pass can paraphrase it away. A window with no vocal in it gets "relaxed closed lips" instead. `compose_shot_prompt` applies it at render time as well, uncapped, so a board planned before the law still reaches the model with the contract on it. `still` as an adverb is deliberately not matched — this list blocks a render, and "the coffee is still hot" must not.
>
> **The biggest face leads the singing.** Face-tight framing did not move the correlation metric (the metric rewards timing, not readability) but it is the most perceptually convincing clip: 100% face detection and the highest mouth articulation of the non-stem candidates, because a mouth is legible at 46% of the frame and mush at 23%. A picture may carry `face_frac` and the biggest measured face opens the rotation. The number is never invented — aspect and short-side cannot tell a 1:1 crop of a face from a 1:1 crop of a room — so an unmeasured picture keeps its given place. A `use` pin or the new per-shot `shot_images` pin wins outright, and moves who is in frame and nothing about the clock.
>
> Commits `f3a77d0` `dd49675` `68da47c` `09ac13e`. Gate: `test_music_video.py test_music_video_routes.py test_storyboard.py test_storyboard_film.py test_routes.py test_no_duplicate_defs.py test_panel_assets.py test_a2v_sync_fixes.py` — **349 passed**, plus `test_a2v_audio_adhesion.py test_storyboard_planner.py` and `node scripts/lint_webapp.mjs` clean. **No GPU was used**: every number above is from the earlier study run, and nothing here was re-rendered.
>
> **Not done, deliberately:** demucs is not installed and the installer script has not been run; there is no panel control for the stem yet (the fields are on the job and on the plan route, the Audio tab has no picker); `face_frac` has no producer yet — the captioner is where it should come from.

> **🎬 2026-09-21 — WHAT THE FIRST REAL MUSIC VIDEO HAD TO BE PLANNED AROUND BY HAND (branch `music-video-fixes`, UNRELEASED).** A whole film went through `/music/video/plan` for the first time, and everything the operator had to do in a scratch script instead of in the panel is a gap. Five of them, closed.
>
> **The bug.** `cells_from_lengths()` read the SECONDS of every `LTX_LENGTHS` row and ignored the row's `qualities` column, so the 20 s / 481-frame cell — offered at Quick alone, because 640×480 holds it and 1024×576 dies around frame 454 (issue #46) — was on the duration axis of a board whose delivery pass is Standard. A long vocal section therefore planned 481-frame **a2v** shots, the most expensive shot this engine makes, none of which can finish on that canvas. The axis is now cut to the pass that DELIVERS the film (`storyboard_final_quality`, `standard` out of the box), in two places that cannot disagree: `cells_from_lengths(lengths, quality=…)` filters the panel's own table, and `plan_music_video(quality=…)` re-applies the restriction to whatever cells it is handed — because the caller who passes a hand-written axis is exactly the caller who can pass a dead cell. `quality` defaults to `standard`, so an unqualified plan is the conservative one. A dropped cell is a note in plain words, and the quality used is echoed back and stored on the board.
>
> **The song's structure is a parameter now.** `song_sections()` reads a score when there is one and guesses from band energy when there is not; neither could know that the first 8.5 seconds are held on an empty stage on purpose, or where a line really lands on a take that was *sung* rather than written. The first film's sections were typed against measured whisper onsets in a scratch script. `POST /music/video/plan` takes `sections` (JSON `{start, end, kind, label?}`), and `lyrics` / `score_abc` as alternatives to the sidecar. Precedence, each rung reached only when the one above is silent: explicit `sections` > form lyrics/score > sidecar > classifier. `given_sections()` sorts, closes gaps, clamps to the song, refuses overlaps and a missing `kind` by name — and always pulls the first section to 0:00, because the film plays from 0:00 with the song under it and a cursor starting at 8.5 s would render every mouth 8.5 s off its words.
>
> **B-roll casting was a blind round-robin**, so the picture the film opens and closes on was cast by accident and the operator re-cast twelve shots by index. A picture now carries `use` (`open` = the first B-roll shot, `close` = the last, `any`) and `weight` (1–20, spread across the rotation in passes rather than clumped), and `shots` carries a per-SHOT prompt override — `[{"n": 24, "prompt": "…"}]` or `{"24": "…"}` — so the last shot can say "the stage lights slowly fading down to black" while the same wide is used three times earlier without a fade. The pins are a post-pass: which shot is "last" is not known until the tiling is done, and a pin moves who is in frame and **nothing** about the clock (durations, `film_start` and every `audio_start_time` are byte-identical with and without pins, asserted).
>
> **The refusal says where files go.** Containment is unchanged — a form field naming a file resolves inside the outputs folder or the uploads folder or it is not used — but "picture 3 is not a file this panel has" now names both folders and `POST /upload` (multipart, field `image`/`audio`), which already existed; no second upload route was added, and a test asserts the route the error points at is a route this panel has.
>
> Gate: `test_music_video.py test_music_video_routes.py test_music_video_minshot.py test_routes.py` — **96 passed** (60 before). No JS changed. Docs: the music-video section of `docs/MUSIC_ENGINE.md` now carries the precedence table and every new field.

> **🎧 2026-09-21 — YuE2 CAN LISTEN: the real-audio tokenizer head, ported (branch `yue2-tokenizer`, UNRELEASED).** The previous entry ends by naming what was deliberately left out — `tokenizer_head_joint_v9`, the thing `nar_lora_joint_v9` was trained beside. It is in. A real recording now becomes YuE2 semantic codes on this Mac, which unlocks a round trip (recording → codes → YuE2's own decoder → audio) and a **continuation** (a song written on from a real take's codes). Owner: *"the tokenizer is out, so it can be trained… do an even better implementation."*
>
> **Nothing about the preprocessing was guessed.** Mothersuperior's `prep_real.py` and `joint_v6.py` were fetched from the release repo and read before a line was written, and every step is theirs: 30-second chunks encoded independently, `hidden_states[20]` concatenated and linearly resampled to `round(seconds × 25)`, rounded through **float16** because that is the precision the head was trained on, instance-normalised **per track** with numpy's population std, 512-frame windows with the learned `pos` sliced rather than interpolated, hop 256 with 128 frames trimmed off every interior edge, and plain `argmax` — the `sem_nbr_*` tables the repo ships are a training soft-CE loss, not an inference step, and they are pinned but unused.
>
> **The two things a port gets silently wrong, both settled against a reference instead of argued.** (1) The fused q,k,v projection: PyTorch's `in_proj_weight [1536,512]` is q,k,v by rows and **every permutation loads without complaint**. Against a torch `TransformerEncoderLayer` built from these exact tensors, q,k,v gives **max-abs-diff 3.910e-05** on a 512-frame window (relative 3.40e-06, argmax agreement 1.0000) and the five wrong permutations give 15.1–17.5. (2) The MERT layer: transformers collects a hidden state only AFTER each conformer block, so `hidden_states[20]` is the output of `layers[20]`, while `lyra.mert`'s `layer_weights` vector is one entry longer because index 0 is the pre-layer embedding — **the one-hot goes at 21**. Against the real HF model: index 21 reproduces `hidden_states[20]` to **1.135e-04**; 19, 20 and 22 are off by 7.3, 12.2 and 19.8. Code statistics do NOT separate them (layer 19 looks just as healthy — 594 unique, repeat 0.072), which is exactly why the reference run mattered.
>
> **The round trip, with controls.** 30 s of a real record (seconds 60–90), seed 4242, 32 ODE steps, measured against the SAME excerpt: stock decoder **7.935 dB** log-mel / chroma **0.8832**; `nar_lora_joint_v9` in `joint` **7.861 dB** / 0.8171; in `separate` 10.389 dB / 0.8314. The controls are what make those numbers mean something: the excerpt against itself is 0.000 dB / 1.0000, a **different** 30 s of the same record is 13.399 dB / 0.702, white noise at the same level 26.283 dB / 0.184, silence 101.5 dB. So the round trip lands inside "the same passage" and nowhere near "a different passage". Encode cost: **2.96 s for 30 s**. Synthesis 13.5–14.8 s, MLX peak 10.48–10.55 GiB. Codes for that excerpt: 750 frames, 565 unique, top code 0.9 %, repeat 0.087, mean run 1.09.
>
> **`--audio-prompt` on the runner.** The recording's first seconds become codes; the codes join the request prefix as ordinary codec tokens, so the AR model prefills on them exactly as on its own output. The codes stay at the head of the token stream and the plan keeps its canonical prefix, which is what `synthesize()` validates against the request. Listening happens before the generator is built and the MERT frontend is dropped after — same order and the same reason as the cover path (2.5 GB beside 10 GB is the memory guard). Measured at seed 4242, `cot=off`, 10 s prompt: **39.52 s of audio in 37.59 s wall** (3.96 s of it listening), ended naturally, against 30.00 s in 30.18 s for the same seed with no prompt (which hit the budget instead). The prompted song's first 10 s reconstruct the source — **10.503 dB / chroma 0.8752** — and its generated tail tracks the prompt's harmony at **chroma 0.5827** against **0.1011** unprompted. The two generated token streams agree on **0.14 %** of frames: a different song, not a perturbed one.
>
> **Two honest edges, in the docstring and the docs.** The repetition-penalty window and the minimum-length counter both restart at the continuation boundary, because `generate_tokens()` takes no prior history — the model sees the prompt, the sampler does not. And instance norm is per track, so the same seconds encode differently depending on what they were cut out of: the same 10 s scored inside a 30 s track agrees with its standalone encode on **34.40 %** of frames. That is faithful to the recipe; it means a prompt is cut once and reused.
>
> **The pack.** The head and the two `sem_nbr` tables go into `mlx_models/yue2-loras/**tokenizer/**` — under a subdirectory on purpose, because `pack_adapters()` globs the root and `user/` and a head beside the adapters would appear in the studio's LoRA picker as if it were one. `lora_problems()` still speaks only about adapters, so an installed pack is not declared broken by a new optional download; `head_problems()` is its own gate. FP32 throughout rather than the training-time bf16 autocast — the two agree on 97.07 % of frames, and polyphase-vs-ffmpeg resampling on 94.67 %.
>
> Commits `76480f3` `eab2b03` `2149d6a` `f0f5810`. Gates: `test_yue2_tokenizer.py` **54 passed** (and `test_yue2_lora.py` 32), `release_gates.sh --fast` **117 PASS / 2 FAIL / 2 SKIP** — both failures pre-existing and unrelated: `test_engine_env_fault` asserts a `site-packages/ltx_pipelines_mlx` string that is absent from `pinokio.js` at the branch base too, and `assert_registry` fails because this worktree has no LTX-2.3 weight pack on disk.
>
> **Not done, deliberately:** no panel surface (runner and CLI only — the studio has no "continue this recording" control); no training. **What training would need on MLX**, estimate only: (a) an **MLX Oobleck VAE encoder** — the weights are already on disk (218 `encoder.*` tensors in `mlx_models/yue2/vae/model.safetensors`) but `lyra/vae.py` is decoder-only, and without it `lat.npy` cannot be made on this Mac at all; (b) a **differentiable NAR path** — `joint_v6` backprops the flow loss through the acoustic branch while the 28-layer AR runs no-grad to build the KV cache, so `lyra.ar`/`lyra.nar` need a BF16 training twin (MLX has what it takes: `mx.grad` works through `mx.fast.scaled_dot_product_attention` and `mx.checkpoint` exists, but `StaticKVCache` and the quantised projections are inference shapes); (c) the **minted anchor corpus** — 4,000 YuE2-corpus tracks with `semantic.npy`, `latent.npy` and `request.json` feed the head's soft-CE every step and are what keep it inside YuE2's dialect; they are not published, and head training on a handful of real tracks without them should be expected to drift. The audio-domain loss additionally needs a differentiable log-mel + multi-resolution STFT through the frozen decoder, which is `mx.fft` plus a mel filterbank (there is no torchaudio here). Rough shape of the work: (a) 1–2 days, (b) 3–5 days, (c) unresolved without upstream data. The head itself is small (~45 M parameters) and would not be the bottleneck.

> **🎛️ 2026-09-21 — LoRA INFERENCE FOR YuE2, AND INSTRUMENTAL BECOMES A MECHANISM (branch `yue2-lora`, UNRELEASED).** The owner's standing bar: *"nothing you can do in Comfy that you cannot do in Phosphene"*. Maestro v2.3.0 had shipped YuE2 LoRA inference and an Instrumental mode built on Mothersuperior's AR adapter; our MLX engine had no adapter code at all, and Instrumental was a prompt convention — a skeleton of bracket tags plus "instrumental, no vocals" in the style.
>
> **Where the code had to live.** `lyra` is installed from a pinned SHA by `music_sync.sh`, so anything written inside `yue2-mlx/` dies on the next sync. The adapter code is ours: `scripts/music/yue2_lora.py`, applied to the models `lyra` hands back. `y = W x + strength · B (A x)`, BF16, at inference, never merged — the AR model runs 8-bit here and a merge would mean BF16-only (5 GB more resident) or a dequantise/requantise per song.
>
> **The one thing that was not obvious.** A wrapper *module* holding the original linear as a child renames every parameter (`…q_proj.weight` → `…q_proj.base.weight`), and `lyra.nar.load_nar()` validates the shared AR model by walking `tree_flatten(parameters())` against the exact upstream names — so the acoustic model would refuse to load halfway through the first song. The projection object is kept and its **class** is swapped for a generated subclass; the matrices hang off the instance through `object.__setattr__`, the pattern `lyra` itself uses. Parameter tree identical, `QuantizedLinear` unchanged, removal is one `__class__` assignment. `LoRARuntime` wraps `_load_model`, the only hook that sees the planning AR, the BF16 conditioning AR *and* the acoustic model built mid-song.
>
> **Modes, Maestro's semantics.** `joint` adds deltas and leaves the decoder projections alone even when the file carries them (`nar_lora_joint_v9` does; the sidecar records `decoder_io_ignored`). `separate` treats the acoustic file as a complete decoder companion and replaces `vae2llm`/`llm2vae` with a **convex blend** across companions — summing them doubles the shared decoder adaptation. Key spellings read: Mothersuperior, `model.`, PEFT/AI-Toolkit `base_model.model.`, `.lora_A.weight`/`.default.weight`, kohya `lora_down/up`, `.A`/`.B`; a PEFT `alpha` folds into B as `alpha/rank`. **The brief's own note that the AR file is MLP-only was wrong** — the header says 392 tensors: 28 layers × `self_attn.{q,k,v,o}` + `mlp.{gate,up,down}`, rank 64, 196 targets.
>
> **Instrumental is two recipes now, and one file decides which.** With the pack: the adapter at 1.0, `cot=full` forced, `[instrumental]` (or bare, optionally timed section tags) in place of words, other LoRAs paused, stock decoder. Without it: the v4.16 skeleton that was validated by ear. A queued song never fails on a missing optional download.
>
> **Proven with audio, not claims.** Four 30-second songs plus a UI job, same seed per pair, only the adapter differing, vocal content measured as demucs `htdemucs_ft` vocal-stem RMS over mix RMS. On a prompt that names a singer with `[instrumental]` as the lyrics: **-9.15 dB without the adapter, -58.53 dB with it — 49.4 dB less voice at the same seed.** On an *instrumental* prompt the pair proves nothing and is reported as such (both arms ≈ -67 dB; the base model already wrote no vocal line, so there was nothing to remove). `nar_lora_joint_v9` loads, synthesises and decodes cleanly and changes the audio, but barely moves the vocal share (0.4888 vs 0.5000) — it was trained beside `tokenizer_head_joint_v9`, which is not ported. Cost per adapter: **-17 %** AR planning throughput, **-16 %** AR semantic, **+12 %** acoustic, **+0.13 GiB** MLX peak. Wall-clock is dominated by how long a score the planner writes, not by the adapter.
>
> **End to end through the UI** on a test panel (`:8474`, isolated state/outputs, Pinokio-style env, stopped by PID): Instrumental on, 30 s, Draft → job `j-1a0c2b270ef-002` done in **39.6 s**, sidecar carrying `instrumental_recipe: mothersuperior-ar-lora` and the adapter's sha256, Song card badged `instrumental · lora` and `ar lora inst v3abc 1.00`, score engraved. That output measures **-56.81 dB** of vocal content. One honest artefact first: the *first* click failed instantly with the GPU-lock refusal, because this session was holding both locks for its own measurements — the refusal did its job and named the lock.
>
> **The pack** (`scripts/pinokio/music_lora_fetch.py`, opt-in, ~210 MB, checksummed, `NOTICE` beside it, `user/` scanned for anything dropped in) and the studio's **Voice & style LoRA** picker, plus a one-line download affordance under the Instrumental pill. `GET /music/lora/status`, `POST /music/lora/fetch`. Commits `f1b1584` `32f7ab8` `318a919` `8cf1fd3` `08578ae`. Gates: `test_yue2_lora.py` 32 passed (wrapper arithmetic checked against hand-written `W x + s·B(A x)` on a plain *and* a quantised base; a wrap renames no parameter), and the touched suites green.
>
> **Not done, deliberately:** the tokenizer head (`tokenizer_head_joint_v9`: 8-layer encoder, d=512, MERT layer-20 features at 25 Hz → 32768 codes — phase 2, the port plan is in the PM hub notes); `separate` bundles have tests but no real artist bundle to hear; the `_comfyui` block-diagonal fused layout is not read.

> **🔍 2026-09-20 (late) — THE BIG REVIEW: Codex, one pass, eleven findings, all fixed (`1fdc030`).** The owner: *"have codex do one big review after finish to find biggest bugs."* Codex CLI 0.155.1 (gpt-6-astra, xhigh), `codex exec -s read-only` with the brief in `/tmp/codex_review_prompt2.md` and the diff named as `git diff review-base...HEAD` (branch `review-base` = the public v4.15.1 tag; `codex exec review --base` refuses a custom prompt). It reproduced four of them in memory before reporting. Ranked as it ranked them:
>
> - **P1** every cover failed *after* publishing its song — the transcription work folder sat inside the artifacts folder and `save_artifacts()` insists on an empty one. Beside it now.
> - **P1** a sub-second B-roll scrap became a 1 s shot in the board writer while every singing shot after it kept its planned offset (singer at 11.08 s, words from 10.42 s). The planner folds scraps into the previous B-roll shot and never writes under `MIN_SHOT_S`; `test_music_video_minshot.py` walks the cursor.
> - **P1** the lyrics / section / Simple routes ran the warm helper without `_GPU_LOCK` — a queued music job kills the helper mid-rewrite, a rewrite mid-render spawns a second MLX model beside YuE2. `_helper_run()` takes the lock with a 3 s timeout and says so.
> - **P1** a cover's source path was not an exact-string secret, so `error_signature` carried `"<path> unreleased demo.wav"`. It is, with `music_parent` and `music_abc_path`.
> - **P2** Update's `uv sync` removed the cover's libraries while readiness stayed green (the extra is on every sync); the beat map's own ffmpeg resolver did not know Pinokio's folders (the planner is imported lazily and pointed at the panel's ffmpeg — which also takes numpy off the boot path); a 4-bit pill for weights the pack does not ship; Write after Cover still submitting the recording; grandchildren missing from the library; a section rewrite landing on the wrong index.
>
> Gates after: 323 passed across the touched suites, lint clean, dry-run 56/56 — checked on their real exit codes this time.

> **🎬 2026-09-20 (evening) — TWO OPUS AGENTS, MERGED: the lyrics editor + Simple mode, and the Music Video pipeline (UNRELEASED, dev).** The owner: *"use Opus agents and get it where it needs to be"* and *"I give it all the images, and then it creates a compelling video out of them."* Each agent worked in its own worktree from the briefs in the PM hub (`notes/yue2/STUDIO_DESIGN.md`, `notes/yue2/MUSIC_VIDEO_BRIEF.md`); I validated each against the real engines they could not reach, then merged.
>
> **Studio UI (`dfb35a8` `15f4d08` `3b69b0e`, + `71d4c69`).** The lyrics are sections now — label pill, lines, count, per-section **Rewrite** (`/music/lyrics/section`, a `section` param on the helper's `write_lyrics`), reorder, delete, `+ section`, a Sections/Text toggle that re-parses on blur, `#musicLyrics` still the one source of truth. **Simple mode**: one description → `/music/simple` → the helper's new `write_song` action returns `{title, style, lyrics}` from one strict-JSON Gemma call, shown in the Custom fields before anything renders. Verified live: 11 s for a title, a style line and full tagged lyrics; a chorus rewritten in four fresh lines. The agent could only stub Gemma, and the first real call came back numbered — `[Verse 1]` — so `music_tags.py` normalises every lyrics path to YuE2's bare tags (Hook → Chorus, Solo → Instrumental, unknown left alone), with tests. The agent also found `cz-section`/`cz-sum` had never had CSS: Cover a song and Advanced had shipped as bare `<details>`.
>
> **Music Video (`fa0c620` `ae44e99` `6be668b`).** `music_video.py`: `song_sections()` reads a YuE2 song's sections as arithmetic — bars × tempo from the score's `M:`/`Q:` and the lyrics' tags say who sings — and falls back to 8-bar downbeat phrases + a band-energy vocal classifier for a foreign recording; `plan_music_video()` turns vocal sections into a2v singing shots (alternating Singer pictures, the largest LTX cell inside 10–20 s) and everything else into 3–7 s i2v B-roll cut on downbeats, **with the film as the cursor** so every `audio_start_time` is the second that will actually be playing under that clip. The plan is an ordinary storyboard board. Routes `/music/video/plan` and `/music/video/film` (song = the bed, `replace` mode). Two real bugs found in the storyboard on the way: **`shot_to_job` never carried `audio`/`audio_start_time`/`image` for an a2v shot** (an a2v shot on a board enqueued as a silent t2v), and **`_sb_export` dropped `music`/`music_mode` in the whole-clip branch** (a soundtrack accepted at the door, a film written without it). One deliberate deviation from the brief: the film route leaves `auto_edit` OFF by default, because `plan_cut` snaps cuts backwards and the accumulated shift slides a2v clips off their own words — the board is already on the grid. Drive video became **Music video** (song, pictures with roles, one Look line, Plan the video → the board opens in Storyboard, Film with the song); the single-clip form is a collapsed "One shot" underneath.
>
> **The merge went wrong twice, both mine.** (1) The two worktrees had symlinks to the real `yue2-mlx`, `node_modules`, `ltx-2-mlx`, `mlx_models` so tests could run; `.gitignore` had `yue2-mlx/` and `node_modules/` with a trailing slash, which ignores a directory and not a symlink — the Music Video agent's `git add -A` committed two of them, and the merge **replaced the real engine checkout + venv and node_modules with self-referential links** (git treats ignored trees as expendable). Rebuilt from the installer's own scripts, both spellings ignored now, memory written (`feedback_agent_worktree_symlinks`). (2) Both agents appended routes to `panel/routes_music.py`; keeping conflict hunks in order interleaved one block inside the other's function — it compiled and 19 route tests failed. Rebuilt as studio-ui's file + music-video's additions. And the gates had been chained through `| tail`, so a red merge reached beta; `0783af5` repairs all of it.
>
> **First end-to-end music video: DONE.** `lights-on-unplugged_film.mp4`, 45.0 s, 8 clips at 1024×576, the song as the bed (h264 + aac 44.999 s). Timings on the M4 Max at the board's "standard" quality: B-roll i2v shots 61–167 s each, the 10 s a2v singing shot **1160 s** — lip-sync is the expensive part, and it is one shot in eight. Frames pulled at 8 s (the room B-roll) and 36 s (the singing close-up, mouth open on the verse) confirm the plan's placement. Nothing was hand-edited. Pictures came from mflux (the singer at an upright with a vintage mic, piano hands with rings, a candlelit room with a guitar on a stand).
>
> **Bohemian Rhapsody, the honest arc.** First attempt (instrumental test) came back with a female voice singing syllables: YuE2 has no instrumental switch — a supplied score with a *Vocal* voice is something to sing, and the mix transcription blends piano and voice into one line. Measured, not guessed: same key Bb, 73 vs 71 BPM, sung-line pitch classes 1.00 vs the source — the notes were the song's, the sound was not. Second pass, per the owner ("get it as close as you can, correct lyrics, I don't care how"): **words** — a local speech model (mlx-whisper large-v3-turbo) on his own mp3, 155 words over 0–151 s, sections at the two breaths (51.5 s, 105.5 s); cross-checked against an online copy (0.90 sequence agreement) and the online text used for the render, never printed; **score** — a `melody-vocal` transcription (the sung line only, 74 lines, Bb, 71); **style** — male operatic tenor, Unplugged; two Final takes at 2:29 / 2:27, both ended naturally; and a true instrumental from the score with the Vocal voice stripped (16 Ins lines, 0 vocal — it ended itself at 1:05). **The voice**: YuE2 cannot clone; the lever is Seed-VC (voice-lab, setting B) on the cover's demucs-separated vocal against Freddie's own demucs-isolated vocal (the solo ballad passage 56–70 s), remixed over the cover's accompaniment — runner at `/tmp/freddie_convert.sh` for now; belongs in the studio as a "Voice" step once judged by ear.

> **First end-to-end music video was rendering as this was first written**: three mflux pictures (a singer at an upright with a vintage mic, piano hands with rings, a small candlelit room with a guitar on a stand), "Lights On" (45 s), plan → 8 shots (7 B-roll, 1 singing at 31.9 s) → render → film with the song. Result in the next entry.

> **🎼 2026-09-20 — MUSIC STUDIO: the song is the unit now, not the file (UNRELEASED, dev).** The owner's brief: *"take it as far as possible... like motherfucking Suno, the Phosphene Music Studio... think about what we are missing in terms of its fractured structure, what tools we can give."* What was fractured: Compose was a form that dropped a WAV into the gallery and forgot everything about it. What the engine could already do and nobody exposed: keep the plan, the tokens, the latents and the noise, and restart from any of them.
>
> **Every song keeps its artifacts** (`state/music/<job-id>/`) and the sidecar names the folder. That single decision is what every verb below stands on. Proven end to end on a spare port, all timed on the M4 Max against a 45-second Draft song:
>
> - **New take** — same score, same words, a new performance. `SymbolicPlan.load` → `replace(request, seed=new)` → the semantic stage again. **21 s.**
> - **Re-roll the sound** — same performance, a new recording of it. `load_artifacts` → fresh solver noise → NAR + decoder only. **12 s** — a quarter of the song. This is the cheap variation nobody else has, because nobody else keeps the tokens.
> - **New style on this score** — the saved `score.abc` as `--abc-file` under the composer's new style and lyrics.
> - **Cover it** — loads the song as the source in Cover a song.
> - **Get the score** — transcription only, a queue job filed under `state/music/scores/`, fetched by job id. **7.5 s** for a 30 s excerpt.
> - **Write lyrics** — a `write_lyrics` action on the warm helper: the resident Gemma 3, a lyricist system prompt written around YuE2's own format (bracket tags, one line per row, a bare tag is instrumental, `[Verse]` not `[Verse 2]`). **9 s**, and the chorus came back the same each time it returned.
> - **Edit the score** — the ABC in a textbox, *Preview* redraws it, *Render this score* queues it. The edited text is written under `state/music/edits/` and the worker refuses any path outside that tree.
>
> **The Song card** (`#songCard`, `webapp/js/music.js`) sits under the player for songs only: title, style, badges (cover / new take / re-rolled / restyled / instrumental / cut at max length), **sheet music engraved by abcjs** (vendored, MIT, `webapp/vendor/abcjs/` — a score has to render with the network off) with key · meter · BPM read off the header, the lyrics with their tags, the lineage link back to the parent, and the four verbs. A song that kept nothing (made before this) gets its buttons disabled with the reason on hover, not a refusal after a click. Gallery cards read like songs: the title, and a chip for what it is.
>
> **The composer became a studio:** three task chips (Write / Cover / Get the score), a title, a concept box with *Write lyrics*, section-tag chips that insert at the caret, **Takes** (N songs from one form, each its own seed), and an Advanced section for what the engine always had — `cfg_scale` (1.0 sent as unset so the sidecar's `null` stays truthful), precision bf16/8bit/4bit.
>
> **Routes** (`panel/routes_music.py`): `/music/variation`, `/music/transcribe`, `/music/score`, `/music/score/render`, `/music/lyrics`. Every one takes OUTPUT paths, never state paths, and `_song_output()` accepts only a file inside the outputs folder carrying a music sidecar. **Lineage** lands in the sidecar (`lineage.parent`, `lineage.variation`) and `list_outputs` attaches a `music` block per song (title, variation, parent, has_score, has_artifacts) so the card's first paint needs no second request. Variation ETAs scale the song estimate (sound ×0.35, take ×0.9).
>
> Two things the lint gate caught on the way: `musicCoverInstall` was never published to the page in the cover commit (an unclickable install button), and the GET handler signature is `(h, parsed)`, not the POST one. 21 new tests + the earlier 14. Docs: `docs/MUSIC_ENGINE.md` gained a Music Studio section.
>
> **Same day, second pass — the Library.** The owner: *"the user interface is really, really far away from something I will be proud of... study Suno pretty well... learn from their best experience."* Studied (help center, release notes to Aug 2026, two walkthroughs) and written up in the PM hub as `notes/yue2/STUDIO_DESIGN.md` — what to keep (a list, cover art on every song, one ⋯ menu carrying every verb, families nested like versions, a persistent player with a real waveform, the lyrics as a document), what not to (two clips per generation as a default cost on a Mac; a social feed), and what only we have (the score). Shipped from it: **the Audio filter renders a song list, not a grid** (`renderSongList`: cover art deterministic from a hash of title+style, title, style chips, duration, badges, a play button, a ⋯ menu — Play / Open / New take / Re-roll / New style / Cover / Edit the score / Show in Finder / Delete, with the impossible ones disabled and the reason on hover); takes nest one level under their parent; **the player surface shows a hero** (cover, title, style, length) instead of a black box with a stock control; and **a sticky player bar** at the bottom of the outputs column — one `<audio>` for every song, prev/next through the list, and a waveform decoded once per song (480 peaks, cached) with the played part in the accent, click to seek. The score pane is capped at 380 px so a long sheet never buries the library.
>
> **Still ahead, in value order** (the plan in the PM hub, `notes/yue2/BEST_IMPLEMENTATION_PLAN.md`): LoRA at inference — the engine has no LoRA code and ComfyUI does, with community adapters ready (`Mothersuperior/YuE2-instrumental-cot-full-loras`); the real-audio tokenizer for true continuation; in-app LoRA training on lora_lab (ComfyUI's is NAR-only and its users report no audible effect — the bar is low); stems; music ↔ storyboard beat-aware cutting.

> **🔧 2026-09-20 — v4.15.1: the fleet's own bug list, fixed. Five faults, every one of them found in the data rather than reported.** The owner's instruction was "check fleet, github and pinokio and solve all bugs, ship fixes together with analytics improvements". GitHub and Pinokio turned out to be almost clean — every open bug thread there is already fixed and waiting on a reporter to confirm (the H3 Q8 SIGKILL in 4.12.2, the dead `madebyollin/taeh3` repo in 4.12.x, the YuE2 MPS flags in 4.14.3, the 48 GB bf16 pick in 4.12.4). **The fleet was not clean**, and nobody had filed any of this.
>
> - **ffmpeg was never where we looked.** Five installs across 4.5.0 → 4.15.0 finished a render and failed the export with "ffmpeg not found on PATH". Every engine spawn has prepended `FFMPEG_BIN` to the child's PATH for months — the miss was one level up: `_resolve_ffmpeg()` knew `pinokio/bin/ffmpeg-env` and nothing else, while **Pinokio's own bootstrap installs `ffmpeg=8.1.2` into its conda prefix** (`bin/miniforge/bin`) and unpacks a private Homebrew beside it. Proven on this Mac: that binary is sitting at `~/pinokio/bin/miniforge/bin/ffmpeg`, 443 KB, dated August. Now searched, along with `bin/miniconda/bin` and `bin/homebrew/bin`; **ffprobe resolves independently** instead of assuming the sibling path exists (the fleet's bare "ffprobe failed:", 3 installs); and a boot warning names every folder that was searched.
> - **A half-built venv looked perfectly healthy.** `ModuleNotFoundError: No module named 'ltx_pipelines_mlx'` on 4.13.0 / 4.14.3 / 4.15.0 — **one install hit it 28 renders in a row.** Both the panel and pinokio.js only ever checked the INTERPRETER, which in that shape exists; what is missing is the package pip writes last. So no repair was offered and nothing said what was wrong until half a minute into each render. `engine_env_fault()` probes the package directory, the panel refuses the spawn in one second with the fix in the sentence, and **pinokio.js now offers "Repair Phosphene engine" for this shape too**.
> - **H3 died eight minutes in on an MLX it cannot use.** `seed(): incompatible function arguments` and `repeat(): …` on 4.14.3 and 4.15.0 — MLX signature drift, neither message naming MLX. H3 needs mlx>=0.32; Phosphene pins mlx==0.31.1 (0.31.2 regresses LTX audio by 22 dB), which is why H3 has its own venv. `h3_mlx_fault()` reads the version off the `.dist-info` directory name — a listing, not a subprocess, because this is on the path to a render — and refuses **before** the LTX helper is killed and before any GPU time is spent.
> - **The captioner would not say which Gemma was wrong.** "Model type gemma4_unified not supported" — 19 events, 4 installs, five releases. Two folders sit side by side in `mlx_models` and only one has a vision tower: `gemma-3-12b-it-4bit` (`gemma3`) and `gemma4-12b-ltx25-q4` (`gemma4_unified`, LTX-2.5's TEXT encoder). mlx_vlm's message names neither the folder it was handed nor the one that works. One read of `config.json` does.
> - **The redactor was eating our own prose.** `~` matched as a home directory on its own, so the fleet is full of signatures reading "this engine needs `<path>`32 GB free RAM" — `~32 GB` is how a person writes an approximation, and that line is the most useful one in the report. A home path always continues with a separator; `~(?=/)` now says so.
>
> **And the analytics half, which is the part that changes what we can see next time.** Both events were proposed on 2026-09-19 and approved with "ship fixes together with analytics improvements".
>
> - **`install_step` — the dark half of the funnel.** 577 installs in 14 days, **44.5% of them ever rendered**, 278 booted once and vanished. Between `app_installed` and the first render the panel said nothing, so "they tried and it was broken" and "they looked and left" were the same shape in the data. Four steps — `first_boot`, `engine_env`, `weights_check`, `first_queue` — each **once per install for all time**, with one deliberate exception: `engine_env` reports again when its answer CHANGES, because a broken environment that gets repaired is the most useful row on the event. `first_queue` is emitted from the worker, the one place eight queue-append call sites converge. No path, no file name, no URL, no host, no byte count, and — unlike `render_failed` — **no signature and no fingerprint**: an install-time error line is the likeliest place in the product for a username-bearing path to survive scrubbing, and the closed class answers the question on its own.
> - **`update_outcome` — the 23 of 139.** People who pressed Update and never ran a new version. `app_updated` cannot show it by construction: a failed update emits nothing. The UI route records the version Update was pressed ON; the next boot compares and emits `running_new`/ok or `restart_pending`/failed, then clears the marker.
> - Both are documented on `docs/ANALYTICS.md` in the same commit, and the dry-run suite's parity guard is what forced that — it failed the build the moment the events existed without the page.
>
> **Tests: 45 new across four files**, and the two fault messages are pinned against the error taxonomy itself, because wording is load-bearing twice over here: "missing (" belongs to `model_missing`, "download"/"fetch" to `download_failed`, and either one files a venv fault under the wrong heading in the fleet view. Both were caught that way, against the live classifier, not guessed.

> **🌍 2026-09-19 — THE GEOIP KILL, REVERSED ON PURPOSE: country and city are now kept, and the page that promised otherwise changed in the same commit (UNRELEASED, dev).** The owner's ruling, verbatim: *"no need to discar i think countrya nd city is fine i wnat to know actualy."* The 2026-08-12 entry below is therefore superseded — not deleted, because three postures in thirteen months is the real record and this file keeps records.
>
> - **Both suppression properties are gone from every event.** `$geoip_disable: True` switched the receiver's GeoIP step off; `$ip: "0.0.0.0"` occupied the property so the connecting address was never copied onto the stored event — and **also killed the lookup**, because the step reads `properties.$ip` and resolves nothing from `0.0.0.0`. That second effect is the one that would have been easy to miss: dropping only the disable flag would have looked like a fix and changed nothing. `_ANALYTICS_RECEIVER_DIRECTIVES` is now one key, `$process_person_profile: False` — counts, still not people.
> - **The `$` namespace is closed to call sites** (`_analytics_clean_props`). While the directives occupied `$ip` and `$geoip_disable`, spreading them last in `_analytics_post` was sufficient armour. It no longer is: a props dict built from job params in code this module does not own could now put a **`$ip` on the wire and spoof the address the GeoIP step reads**, or a `$geoip_city_name` and invent a location outright. Every `$`-prefixed key from a call site is dropped; ours are spread afterwards.
> - **The owner's note had the project setting backwards, and the correction matters.** *Discard client IP data* does NOT suppress geo — PostHog runs the GeoIP transformation FIRST and discards the address after, so **on** = country and city stored, address not stored; **off** = the same country and city *plus* a retained IP on every event. It is still the owner's click (project settings are not ours to change) but it now costs him nothing he wants. `plugin-advanced-geoip` is the lever if he later wants postal code and city-centre coordinates dropped too — GeoIP stores seven properties, and he asked for two.
> - **`docs/ANALYTICS.md` Location section rewritten in the same commit**, which is the only thing that makes a promise like this worth reading: it names all seven stored `$geoip_*` properties with examples, says why they are kept, says the address is discarded at the edge **and that a source tree can state that and cannot prove it**, keeps the 1,304-event / 85-install history (the owner chose to keep those rows), and points at the three off switches. The summary table row changed too — it used to read *"None sent, and every event tells the receiver not to derive one"*.
> - **Guards rewritten, not relaxed, and mutation-tested 3/3 red:** put `$ip` back → red; put `$geoip_disable` back → red; reopen the `$` namespace → red. The surviving invariant is the one that was always the real one: **the panel sends no location field and reads no locale, timezone or coordinate off the Mac.** A derived country is a fact about a connection; a sent one would be a fact this program went looking for.
> - **Caught a stale fixture the pytest run cannot see.** `scripts/test_analytics_dryrun.py` runs standalone (56 tests), so last night's `cancelled_race` tightening in `9c66022` left `"cancel requested, landing as failed"` classifying as `other` with nobody watching. Replaced with the three strings the panel actually raises (`"H3 render stopped early at the next forward boundary"`, `"cancelled by /stop (rc=-9)"`, `"stopped before the join"`) plus a negative case pinning that a message merely containing the word is **not** a cancel. 56/56.
...
## 2026-09-21 night — Robot Freddie Unplugged (private experiment, PM hub notes)

State lives in `~/AI/projects/phosphene/notes/yue2/` (briefs `ROBOT_*`, `OVERNIGHT_2026_09_21_FILM.md`,
studies `MAESTRO_VOICE_STUDY.md`, `GROOVE_STUDY.md`, `MAESTRO_LIPSYNC_STUDY.md` when it lands) and in
memory. Code on dev today: LoRA inference + Instrumental (709a176), tokenizer head + `--audio-prompt`
(ac785db), `--lora-trigger` tests (40f0a5e), music-video planner fixes (601e723) — all pushed to beta,
nothing to public main. Studio test panel :8473 runs dev 601e723 from this checkout (PID `/tmp/studio_pid.txt`).
Overnight agents (song v3, lip-sync A/B, H3 draft film) write only named files into prod `mlx_outputs`.
